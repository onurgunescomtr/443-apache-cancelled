...
final class ClassicContactUs{
    ...
    {
        if (AppAudit::checkPostVars(['giris-istegi','g-recaptcha-response']) && AppAudit::checkOpenKey(Http::__px('giris-istegi'))){
            
            AppAudit::manageRequestCount();
            
            

            if (!AppAudit::checkPostVars($this->contactFormVars)){

                Http::guide(ADDRESS . '/' . App::getApp('contactUri'),'warn',self::infoContactUs[LANG]['form_bos']);
            }
            
            if (AppAudit::checkGoogleCaptSpecial('g-recaptcha-response','iletisimsayfasi')){
    ...
    private function iletisimKaydiAl(): void
    {
        try{
            ...
            $y->kullanici_hesap_no = SysLed::get('user_page_idstring');
            ...
            $y->ip = SysLed::get('user_client_address');
